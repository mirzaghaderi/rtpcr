...
The rtpcr package does not automatically handle missing Ct values. However, NA can be used to represent missing Ct measurements. In such cases, the corresponding data for that sample will be produced as NA and passed along to downstream statistical analyses. If more sophisticated handling of missing Ct values is desired (for example, concidering as fixed values or imputing), qPCR imputation tools can be used in advance of analysis with rtpcr.
