...
- Possibility of using shiny version of the rtpcr package as described
  in the manual.
...
- [`ANOVA_DDCt()`](https://mirzaghaderi.github.io/rtpcr/reference/ANOVA_DDCt.md)
  ...
  `specs = "FactorA | FactorB"`). As analysis across interactions of
  multiple factors output was not adopted with the `plotSingleGene()`
  function, this function was removed.
...
- A function called `plotSingleGene()` was added that creates a bar plot
  of relative gene expression (fold change) values from single gene
  analysis showing all pairwise significances.
