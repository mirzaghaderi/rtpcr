...
![](../man/figures/standCur.png){.center height="200px"}
...
![Figure 5: Calculation of standard error (se) for ddCt–based relative expression in the `ANOVA_DDCt()` function of the rtpcr package. The schematic illustrates how weighted dCt (wdCt) and weighted ddCt (wddCt) values are combined under different experimental designs, and how the standard error is computed depending on the `se.type` argument. `"paired.sample"` computes se from paired differences (used when a random id effect is present), `"two.sample"` uses the unpaired two-sample t-test standard error against the reference level, and `"single.sample"` computes se within each level using a one-sample t-test. For independent samples, `ANOVA_DDCt()` automatically uses `se.type = "two.sample"`, and if repeated‐measure or paired design model is specified, `ANOVA_DDCt()` automatically selects `se.type = "paired.sample"`](../man/figures/repeated_measure.png){.center height="440px"}
