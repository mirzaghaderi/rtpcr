...
method described by Ganger et al. (2017) and Taylor et al. (2019), covering
...
2. Replicates information (biological replicates or subjects; see [NOTE 1](#note-1), and [NOTE 2](#note-2))  
...
#### NOTE 2
Your data table may also include technical replicates. In this case, the `meanTech` function should be applied first to calculate the mean of the technical replicates. The resulting table is then used as the input for expression analysis. To use the `meanTech` function correctly, the technical replicate column must appear immediately after the biological replicate column (see [Mean of technical replicates](#mean-of-technical-replicates) for an example). 
