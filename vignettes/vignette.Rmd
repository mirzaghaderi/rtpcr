...
`qpcrANCOVA` or `qpcrREPEATED.` The values can be returned for any effects in the model including simple effects,
...
`qpcrANCOVA` and `twoFACTORplot` functions give FC results for one gene each time. you can combine FC tables of different genes and present their bar plot by `twoFACTORplot` function. An example has been shown bellow for two genes, however it works for any number of genes.
